...
This section presents the intercept-only beta-binomial phylogenetic model used to estimate overall mixed-paternity prevalence across birds. The model accounts for study identity, non-phylogenetic species-level variation, and phylogenetically structured species-level variation.
:::
...
## Overall mixed-paternity prevalence

### Model

The intercept-only model estimated the average mixed-paternity prevalence across studies and species while accounting for study-level, non-phylogenetic species-level, and phylogenetically structured species-level variation.
...
knitr::include_graphics(
  "Figures/model_diagnostics/overall_prevalence_plot.png"
)
```
...
phylo_signal_summary <- draws_phylo_signal %>%
  median_hdci(phylo_signal, .width = 0.95) %>%
  # Zmiana nazw kolumn i ułożenie ich do ładnej tabeli
  transmute(
    Statistic = "Phylogenetic signal (ν)",
    Estimate = phylo_signal,
    `Lower 95% CrI` = .lower,
    `Upper 95% CrI` = .upper
  )
...
knitr::kable(
  ...
  digits = 3, 
  caption = "Bayesian estimate of the phylogenetic signal with 95% Highest Density Interval (HDI)."
)
```
