  ...
  left_join(
    data_overall_study %>%
      ...
      summarise(
        ...
        n_study_identities_species = n_distinct(Study_ID[!is.na(Study_ID)]),
...
stopifnot(
  ...
  all(species_annotations$n_study_identities_species >= 0)
)
...
readr::write_csv(
  species_annotations %>%
    select(
      species, mixed_broods, total_broods, n_study_identities_species,
...
map_validation <- tibble(
  check = c(
    ...
    "Minimum number of study identities",
    "Maximum number of study identities",
  ...
  value = c(
    ...
    min(map_data_points$n_study_identities),
    max(map_data_points$n_study_identities),
...
```{r}
#| fig-cap: "Global geographic distribution of analytical study identities included in the comparative meta-analysis of avian mixed paternity. Point size indicates the number of distinct study identities associated with each rounded geographic coordinate, and point colour indicates pooled observed mixed-paternity prevalence, calculated as the total number of mixed-paternity broods divided by the total number of broods sampled at that location."
...
Points represent observed record-level mixed-paternity prevalence and are scaled by the number of genotyped broods. The dashed line and dark point mark the inverse-logit-transformed population-level intercept from the intercept-only model; its credible interval is reported in the model table. This conditional estimate sets study and species random effects to zero.
...
#| fig-width: 10
#| fig-height: 5
#| fig-cap: "Observed mixed-paternity prevalence across study populations and the inverse-logit-transformed population-level intercept from the intercept-only model. The 32.5% estimate is conditional on study and species random effects being set to zero and is not a marginal average across observed studies or species."
...
p_overall_prevalence <- ggplot(plot_data, aes(x = observed_prev, y = 0)) +
  ...
  annotate(
    ...
    x = overall_est + 6,
    y = 0.18,
    label = paste0("Population-level intercept: ", round(overall_est, 1), "%"),
    hjust = 0,
    size = 5
  ) +
  scale_x_continuous(
...
forest_dat_refitted <- tibble::tribble(
  ~group, ~moderator, ~estimate, ~lower, ~upper,
...
ggplot2::ggsave(
  "Figures/Forest_plot8.pdf", p_forest_main,
  width = 14, height = 10.5, units = "in"
)
ggplot2::ggsave(
  "Figures/Forest_plot8.png", p_forest_main,
  width = 14, height = 10.5, units = "in", dpi = 300
)
...
#| fig-cap: "Posterior mean effects and 95% credible intervals from the single-moderator models. Continuous predictors are expressed per one standard deviation using the common scaling derived from the overall analytical dataset. For multi-category reproductive classifications, the plot shows the biologically interpretable contrasts reported in the Results; complete pairwise comparisons are provided in the breeding-system chapter."
#| echo: false
#| out-width: "100%"
